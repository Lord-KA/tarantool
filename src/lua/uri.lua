...
local function parse(str)
    ...
    if uri.internal.uri_create(uribuf, str) ~= 0 then
...
local function parse_many(str)
    ...
    if uri.internal.uri_set_create(uri_set_buf, str) ~= 0 then
...
local function fill_uribuf_params(uribuf, uri)
    ...
    uribuf.params = ffi.new("struct uri_query_param[?]", uribuf.param_count)
    local i = 0
